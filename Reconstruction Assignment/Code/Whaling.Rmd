...
ggplot(data_long, aes(x = Year, y = Mortality, color = Type, group = Type)) +
  ...
  theme_minimal() +
  #Changing the legend to account for the countries
  scale_color_manual(
    values = c("Traditional" = "#E41A1C", 
               "Commercial" = "#377EB8", 
               "Scientific" = "#4DAF4A"),
    labels = c("Traditional (Denmark, St. Vincent and Grenadines, Russia, USA)", 
               "Commercial (Norway and Iceland)", 
               "Scientific (Japan)")
  ) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
